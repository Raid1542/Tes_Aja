...
if (isset($_POST['tambah_keranjang'])) {
    ...
    if ($produk) {
        ...
        if ($stok >= $jumlah) {
            $query_keranjang = mysqli_query($koneksi, "SELECT * FROM keranjang WHERE username = '$username' AND nama_produk = '$nama_produk' AND ukuran = '$ukuran'");
            ...
            if (mysqli_num_rows($query_keranjang) > 0) {
                ...
                mysqli_query($koneksi, "UPDATE keranjang SET jumlah = '$jumlah_baru' WHERE produk_id = '{$item['id_produk']}'");
            } else {
                // Jika produk belum ada di keranjang, tambahkan produk baru
                mysqli_query($koneksi, "INSERT INTO keranjang (username, produk_id, nama_produk, ukuran, harga, jumlah) VALUES ('$username', '$produk_id', '$nama_produk', '$ukuran', '$harga', '$jumlah')");
            }
...
<body>
    ...
    <?php foreach ($cartItems as $item): ?>
        <div class="cart-item">
            ...
            <input type="checkbox" name="selected_items[]" value="<?php echo $item['produk_id']; ?>" class="select-item" onchange="updateSubtotal()">
            <form method="POST" action="">
